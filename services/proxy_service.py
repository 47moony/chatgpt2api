...
class ProxySettingsStore:
    ...
    def get_profile(
        ...
        account_proxy = self._account_proxy_url(account)
    ...
    @staticmethod
    def _account_proxy_url(account: dict | None) -> str:
        if not isinstance(account, dict):
            return ""

        proxy_ref = proxy_by_key(str(account.get("proxy_key") or ""))
        if proxy_ref:
            return proxy_url(proxy_ref)
        ...
        if isinstance(account_proxy, dict):
            ...
            if proxy_ref:
                return proxy_url(proxy_ref)
            if not ProxySettingsStore._is_stale_local_proxy(account_proxy):
                return proxy_url(account_proxy)
            return ""

        return _clean(account_proxy)

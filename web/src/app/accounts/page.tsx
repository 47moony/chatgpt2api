...
function displayProxy(account: Account) {
  ...
  if (proxy && typeof proxy === "object") {
    ...
    if (proxy.proxy_key) return proxy.proxy_key;
  }
  return account.proxy_key || "—";
}

function editableProxyValue(account: Account) {
  const proxy = account.proxy;
...
function AccountsPageContent() {
  ...
  const openEditDialog = (account: Account) => {
    ...
    setEditProxy(editableProxyValue(account));
  };
            ...
            <div className="flex flex-col gap-3 border-b border-stone-100 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex flex-wrap items-center gap-2 text-sm text-stone-500">
                ...
                <Button
                  ...
                  disabled={selectedRecoverableTokens.length === 0 || isRecovering || isRefreshing}
                  title="只恢复已勾选、异常、且属于认证失败的账号"
                >
              ...
              <table className="w-full min-w-[1420px] text-left">
                <thead className="border-b border-stone-100 text-[11px] text-stone-400 uppercase tracking-[0.18em]">
                  <tr>
                    ...
                    <th className="w-36 px-4 py-3">凭据归属</th>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1 text-stone-400">
                            ...
                            <button
                              ...
                              disabled={isRefreshing || isRecovering || refreshingTokens.has(account.access_token)}
                              title="刷新账号信息和额度"
                            >
